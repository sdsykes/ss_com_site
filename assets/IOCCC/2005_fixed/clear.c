...
#include <string.h>
#include <curses.h>
int d,C,Z,I,D,B,V,S,i=49152,t,s,o,h,z,O=256,n=65535,l=128,f=255,e,w; 
...
main(int c, char *v[]){ 
  ...
  if (g){
    ...
    noecho();
    for(;;){
      o++ % (c * 4) ? h : (h = getch() , h - ERR ? (m[n & 162] = h | l) : 0);
      if(!w){
	s = (m[n & 59408] |= z ? l : 0) & 15;
	m[n & 59410] =~ (h - ERR ? (s ^ 8 ? 0 : j[h | l] & 1) | (j[h] / 8 - 5 - s ? 0 : 1 << j[h] % 8) : 0);
      }
      o % c | I || N();
      p = m + O + O + m[n & l - 9];
      t = d % 65493;
	  ...
	  *u = 0;
	  ...
	  if (g = fopen(p, t ? j + 61 : j + 32)){
		...
		*p++ = i / O;
	    ...
	    for(; t; fputc(m[n & i], g)) t = m[n & ++i] ? 3 : t - 1;
	    fclose(g);
	  }
	}
	i = 96;
      ...
      e = t-- ? t-- ? t-- ? t-- ? t-- ? t-- ? t-- ? t-- ?
	(t = d , d += 2 , m[n & t] + m[n & t + 1] * O) + (i - 190 ? x : y)
	: (t = d , d += 2 , m[n & t] + m[n & t + 1] * O) + y
	: f & m[n & d++] + (i - 150 && i - 182 ? x : y)
	...
	s = *p % l;
	i = A_REVERSE;
	*p > s ? attron(i) : attroff(i);
	mvaddch(t / 40, t % 40, s + w < 32 ? s + 64 : s > 95 + w ? s - 32 : s);
      }
    }
  }
  perror(*v);
